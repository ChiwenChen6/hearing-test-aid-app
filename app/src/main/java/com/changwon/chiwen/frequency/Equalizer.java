...
//                input_button.setEnabled(true);
                Toast.makeText(context, "以連接", Toast.LENGTH_SHORT).show();
...
//    }
    ...
    int[] save_position = {8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8};   //reset
    int[] load_position = {8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8};   //reset
    int l_average = 0; // average on the left
    int r_average = 0; // average on the right
    int[] left_frequency_band = new int[12];    // 設定12個頻段的值 右
    int[] right_frequency_band = new int[12];   // 設定12個頻段的值 左
    int[] left_gain = new int[12];  //左增益值
    int[] right_gain = new int[12]; //右增益值

    byte[] left_gain_byte = new byte[12];   // 左頻率字節值存儲變量byte將通過藍牙發送
    byte[] right_gain_byte = new byte[12];  // 右頻率字節值存儲變量byte將通過藍牙發送
...
//                scanDevice();
        ...
        save_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                try{scanDevice();} catch (Exception e) {
                    e.printStackTrace();}
...
//                Log.d("Safaud", "輸出");
    ...
    public void Average_Function() {
        ...
        //左頻段陣列 ( 16Hz, 31.5Hz, 63Hz, 125Hz, 250Hz, 500Hz, 1KHz, 2KHz, 4KHz, 8KHz, 16KHz, 32KHz )
        ...
        // 正確的頻段安排 ( 16Hz, 31.5Hz, 63Hz, 125Hz, 250Hz, 500Hz, 1KHz, 2KHz, 4KHz, 8KHz, 16KHz, 32KHz )
        ...
        // 將左右（-12dB至+ 12dB）值轉換為ASCII碼97至121值
        for(int i = 0 ; i < 12 ; i++) {
            left_gain[i] += 109;
            right_gain[i] += 109;
